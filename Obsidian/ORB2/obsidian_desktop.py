...
ROLE_COLORS   = {"ceo":"#f5c518","dg":"#c084fc","admin":ERR,"hr":INFO,"manager":WARN,"staff":OK}
...
class ObsidianApp(ctk.CTk):
    ...
    def _build_tab_payroll(self):
        ...
        # ── Filters toolbar
        tb=ctk.CTkFrame(f,fg_color="transparent"); tb.pack(fill="x",pady=(0,6))
        self._pr_srch=tk.StringVar(); self._pr_srch.trace_add("write",lambda *_:self._refresh_payroll())
        ctk.CTkEntry(tb,textvariable=self._pr_srch,width=220,height=34,
                     placeholder_text="🔍  Search name, ID, dept...",font=self.F_BODY).pack(side="left")

        self._pr_dept_var=tk.StringVar(value="All Departments")
        self._pr_dept_menu=ctk.CTkOptionMenu(tb,variable=self._pr_dept_var,
            values=["All Departments"],
            command=lambda _:self._refresh_payroll(),width=150,font=self.F_SMALL)
        self._pr_dept_menu.pack(side="left",padx=6)

        self._pr_cur_var=tk.StringVar(value="All Currencies")
        ctk.CTkOptionMenu(tb,variable=self._pr_cur_var,
            values=["All Currencies","USD","NGN","GBP","EUR","KES","ZAR","GHS"],
            command=lambda _:self._refresh_payroll(),width=130,font=self.F_SMALL).pack(side="left",padx=3)

        self._pr_st_var=tk.StringVar(value="All Status")
        ctk.CTkOptionMenu(tb,variable=self._pr_st_var,
            values=["All Status","active","inactive","on_leave"],
            command=lambda _:self._refresh_payroll(),width=110,font=self.F_SMALL).pack(side="left",padx=3)

        ctk.CTkButton(tb,text="📤 Excel",fg_color=GOLD_D,width=90,height=34,
                      command=self._export_pay_xl).pack(side="right",padx=3)
        ctk.CTkButton(tb,text="📤 CSV",fg_color=GOLD_D,width=80,height=34,
                      command=self._export_pay_csv).pack(side="right",padx=3)

        self._pr_count_lbl=ctk.CTkLabel(f,text="",font=self.F_SMALL,text_color=TXM)
        self._pr_count_lbl.pack(anchor="w",pady=(0,4))
        ...
        # ── Table
        cols=("ID","Name","Department","Occupation","Role","Salary","Currency","Type","Status")
        widths={"ID":120,"Name":150,"Department":120,"Occupation":130,"Role":90,"Salary":110,"Currency":65,"Type":100,"Status":80}
    ...
    def _refresh_payroll(self):
        ...
        fd  = self._pr_dept_var.get()
        fc  = self._pr_cur_var.get()
        fs  = self._pr_st_var.get()

        # Update dept filter options
        depts = ["All Departments"] + list(self.data["departments"].keys())
        self._pr_dept_menu.configure(values=depts)

        all_emps = list(self.data["employees"].items())
        rows = [(eid,e) for eid,e in all_emps if (
            (not q or any(q in str(v).lower() for v in [eid,e.get("full_name",""),e.get("department",""),e.get("role","")])) and
            (fd in ("All Departments","") or e.get("department")==fd) and
            (fc in ("All Currencies","") or e.get("salary_currency")==fc) and
            (fs in ("All Status","") or e.get("status")==fs)
        )]

        # ── Stats
        usd_sals = [float(e.get("salary",0) or 0) for _,e in rows if e.get("salary_currency")=="USD" and e.get("salary")]
        tot_usd = sum(usd_sals)
        w_sal   = sum(1 for _,e in rows if e.get("salary"))
        hi_usd  = max(usd_sals, default=0)
        lo_usd  = min(usd_sals, default=0)
        avg_usd = round(tot_usd / max(len(usd_sals),1))
        for w in self._pr_stats.winfo_children(): w.destroy()
        self._stat_row(self._pr_stats,[
            ("Total Shown",    len(rows),           INFO),
            ("With Salary",    w_sal,               OK),
            ("Total USD",      f"${tot_usd:,.0f}",  GOLD_D),
            ("Avg USD",        f"${avg_usd:,.0f}",  GOLD_D),
            ("Highest USD",    f"${hi_usd:,.0f}" if hi_usd else "—", OK),
            ("Lowest USD",     f"${lo_usd:,.0f}" if lo_usd else "—", ERR),
        ])

        # ── Count label
        self._pr_count_lbl.configure(text=f"Showing {len(rows)} of {len(all_emps)} employees")

        # ── Dept breakdown (always all employees for context)
        ...
        all_usd = sum(dmap.values())
        for dept,val in sorted(dmap.items(), key=lambda x:-x[1]):
            ddef = get_dept_definition(dept)
            pct  = round(val/all_usd*100) if all_usd else 0
            col  = ddef.get("color", GOLD)
            dr   = ctk.CTkFrame(self._pr_dept_inner,fg_color="transparent")
            dr.pack(side="left",padx=8,fill="x",expand=True)
            ctk.CTkLabel(dr,text=f"{ddef.get('icon','')} {dept}",
                         font=self.F_SMALL,text_color=TXM).pack(anchor="w")
            ctk.CTkLabel(dr,text=f"${val:,.0f}",
                         font=ctk.CTkFont("Courier New",15,"bold"),text_color=col).pack(anchor="w")
            bar_bg = ctk.CTkFrame(dr,fg_color=BORDER,corner_radius=2,height=4)
            bar_bg.pack(fill="x",pady=(2,0))
            if pct:
                ctk.CTkFrame(bar_bg,fg_color=col,corner_radius=2,height=4,
                             width=max(int(100*pct/100),2)).pack(side="left")
            ctk.CTkLabel(dr,text=f"{pct}%",font=self.F_MONO,text_color=TXM).pack(anchor="w")
    ...
    def _build_tab_audit(self):
        ...
        # Stats row
        self._aud_stats = ctk.CTkFrame(f,fg_color="transparent"); self._aud_stats.pack(fill="x",pady=(0,8))
        # Toolbar
        tb=ctk.CTkFrame(f,fg_color="transparent"); tb.pack(fill="x",pady=(0,8))
        self._aud_srch=tk.StringVar(); self._aud_srch.trace_add("write",lambda *_:self._refresh_audit())
        ctk.CTkEntry(tb,textvariable=self._aud_srch,width=220,height=34,
                     placeholder_text="🔍  Search user, action, target...",font=self.F_BODY).pack(side="left")
        self._aud_act_var=tk.StringVar(value="All Actions")
        ctk.CTkOptionMenu(tb,variable=self._aud_act_var,
            values=["All Actions","Login","Added employee","Updated employee","Deleted employee",
                    "Promotion","Leave","Imported","Updated salary","Changed password",
                    "Saved system user","Saved department"],
            command=lambda _:self._refresh_audit(),width=160,font=self.F_SMALL).pack(side="left",padx=6)
        self._aud_usr_var=tk.StringVar(value="All Users")
        self._aud_usr_menu=ctk.CTkOptionMenu(tb,variable=self._aud_usr_var,
            values=["All Users"],
            command=lambda _:self._refresh_audit(),width=130,font=self.F_SMALL)
        self._aud_usr_menu.pack(side="left",padx=3)
        self._aud_dr_var=tk.StringVar(value="All Time")
        ctk.CTkOptionMenu(tb,variable=self._aud_dr_var,
            values=["All Time","Today","This Week","This Month"],
            command=lambda _:self._refresh_audit(),width=110,font=self.F_SMALL).pack(side="left",padx=3)
        ctk.CTkButton(tb,text="📤 Export CSV",fg_color=GOLD_D,width=120,height=34,
                      command=self._export_audit_csv).pack(side="right",padx=3)
        self._aud_count_lbl=ctk.CTkLabel(f,text="",font=self.F_SMALL,text_color=TXM)
        self._aud_count_lbl.pack(anchor="w",pady=(0,4))
        cols=("Timestamp","User","Action","Target","Detail")
        widths={"Timestamp":150,"User":120,"Action":200,"Target":150,"Detail":300}
    ...
    def _refresh_audit(self):
        ...
        act = self._aud_act_var.get()
        usr = self._aud_usr_var.get()
        dr  = self._aud_dr_var.get()

        # Update user filter
        all_log = self.data.get("audit_log",[])
        users   = sorted(set(a.get("user","") for a in all_log if a.get("user")))
        self._aud_usr_menu.configure(values=["All Users"]+users)
        ...
        rows = []
        for a in all_log:
            if act not in ("All Actions","") and act.lower() not in a.get("action","").lower(): continue
            if usr not in ("All Users","") and a.get("user") != usr: continue
            if cutoff:
                try:
                    if datetime.strptime(a["ts"],"%Y-%m-%d %H:%M:%S") < cutoff: continue
                except: pass
            if q and not any(q in str(v).lower() for v in a.values()): continue
            rows.append(a)

        # ── Stats
        for w in self._aud_stats.winfo_children(): w.destroy()
        today_str = today.isoformat()
        today_cnt = sum(1 for a in all_log if a.get("ts","").startswith(today_str))
        unique_users = len(set(a.get("user","") for a in all_log if a.get("user")))
        self._stat_row(self._aud_stats,[
            ("Total Events",   len(all_log), INFO),
            ("Showing",        len(rows),    OK),
            ("Unique Users",   unique_users, GOLD_D),
            ("Today",          today_cnt,    WARN),
        ])

        # ── Count
        self._aud_count_lbl.configure(text=f"{len(rows)} event{'s' if len(rows)!=1 else ''}")

        # ── Table
        for row in self._audit_tree.get_children(): self._audit_tree.delete(row)
        ACTION_ICO = {
            "Login":"🔐","Added":"➕","Updated":"✏️","Deleted":"🗑",
            "Promotion":"🏆","Leave":"📅","Imported":"📥",
            "Changed":"🔑","Saved":"💾","Updated salary":"💰",
        }
        for a in rows:
            ico = ACTION_ICO.get(a.get("action","")) or ACTION_ICO.get(a.get("action","").split()[0],"📋")
    ...
    def _build_tab_sysusers(self):
        ...
        cols=("Username","Full Name","Role","Dept Scope","Employee ID","Created")
        widths={"Username":130,"Full Name":160,"Role":130,"Dept Scope":140,"Employee ID":120,"Created":100}
    ...
    def _refresh_sysusers(self):
        ...
        for uname,d in self.data["system_users"].items():
            ...
            dept  = d.get("department","") or "All departments"
            self._su_tree.insert("","end",iid=uname,values=(
                uname, d.get("full_name",""), rlabel, dept,
                d.get("emp_id","") or "\u2014", d.get("created","")))
    ...
    def _su_dialog(self, uname=None):
        ...
        dlg.geometry("480x560"); dlg.grab_set(); dlg.configure(fg_color=BG)
        ...
        dept_v= tk.StringVar(value=d.get("department","") or "")
        pw_v  = tk.StringVar()
        ...
        for label,var,opts,show in [
            ...
            ("Role",             rl_v,
             ["staff","manager","hr","admin","dg","ceo"], None),
        ...
        # Department scope
        ctk.CTkLabel(dlg,text="Department Scope (restricts Manager/Staff visibility)",
                     font=self.F_SMALL,text_color=TXM).pack(anchor="w",padx=22,pady=(8,1))
        depts = ["— No restriction (all departments) —"] + list(self.data.get("departments",{}).keys())
        dept_menu = ctk.CTkOptionMenu(dlg,variable=dept_v,
                                      values=depts,font=self.F_BODY)
        dept_menu.pack(fill="x",padx=22)
        # Set current value properly
        cur_dept = d.get("department","")
        dept_v.set(cur_dept if cur_dept else "— No restriction (all departments) —")

        rp_lbl=ctk.CTkLabel(dlg,text="",font=self.F_SMALL,text_color=TXM,wraplength=420,justify="left")
        rp_lbl.pack(anchor="w",padx=22,pady=(8,2))
        def _upd(*_):
            rdef=ROLE_DEFINITIONS.get(rl_v.get(),{})
            col=rdef.get("color","#c9a84c")
            rp_lbl.configure(
                text=f"{rdef.get('icon','')}\u2002{rdef.get('label',rl_v.get())} \u2014 {rdef.get('purpose','')}",
                text_color=col)
        ...
        def _save():
            ...
            raw_dept = dept_v.get()
            dept_val = None if raw_dept.startswith("—") else raw_dept
            ...
            entry = {
                ...
                "emp_id":    eid_, "department": dept_val,
            }
        ...
        ctk.CTkButton(dlg,text="\U0001f4be Save",fg_color=GOLD,hover_color=GOLD_L,text_color=BG,
                      height=42,command=_save).pack(pady=14,fill="x",padx=22)
