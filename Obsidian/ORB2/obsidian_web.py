  ...
  <div id="body">
    ...
    <main id="main">
      ...
      <section class="pg" id="pg-payroll">
        <div class="pghdr"><div class="pght"><div class="pgt">Payroll &amp; Salary</div><div class="pgs">Compensation management — HR &amp; Admin only</div></div></div>
        ...
        <div class="card" style="margin-bottom:14px;padding:0">
          <div style="padding:14px 18px;border-bottom:1px solid var(--bdr);font-family:var(--bb);font-size:13px;color:var(--txm)">SALARY DISTRIBUTION</div>
          <div style="padding:14px 18px"><canvas id="pay-chart" height="80"></canvas></div>
        </div>
        <div class="tb" style="flex-wrap:wrap;gap:8px">
          <input class="si2" id="psrch" type="text" placeholder="🔍  Search name, ID, role…" oninput="rPay()" style="min-width:200px">
          <select class="flt" id="pfd" onchange="rPay()">
            <option value="">All Departments</option>
          </select>
          <select class="flt" id="pfc" onchange="rPay()">
            <option value="">All Currencies</option>
            <option value="USD">USD</option><option value="NGN">NGN</option>
            <option value="GBP">GBP</option><option value="EUR">EUR</option>
            <option value="KES">KES</option><option value="ZAR">ZAR</option><option value="GHS">GHS</option>
          </select>
          <select class="flt" id="pfs" onchange="rPay()">
          ...
          <div class="sp"></div>
          <span id="pay-count" style="font-size:12px;color:var(--txm);align-self:center"></span>
        ...
        <div class="tw"><table><thead><tr>
          <th>ID</th><th>Name</th><th>Department</th><th>Occupation</th><th>Role</th>
          <th>Salary</th><th>Currency</th><th>Type</th><th>Status</th>
        </tr></thead><tbody id="prtb"></tbody></table></div>
      </section>

      <!-- AUDIT LOG -->
      <section class="pg" id="pg-audit">
        <div class="pghdr"><div class="pght"><div class="pgt">Audit Log</div><div class="pgs">Complete system activity trail — HR &amp; Admin only</div></div></div>
        <div class="sg" id="auditstats" style="margin-bottom:14px"></div>
        <div class="tb" style="flex-wrap:wrap;gap:8px">
          <input class="si2" id="asrch" type="text" placeholder="🔍  Search user, action, target…" oninput="rAudit()" style="min-width:220px">
          <select class="flt" id="afl" onchange="rAudit()">
            <option value="">All Actions</option>
            <option value="Login">🔐 Login</option>
            <option value="Added employee">➕ Added Employee</option>
            <option value="Updated employee">✏️ Updated Employee</option>
            <option value="Deleted employee">🗑 Deleted Employee</option>
            <option value="Promotion">🏆 Promotion</option>
            <option value="Leave">📅 Leave</option>
            <option value="Imported">📥 Import</option>
            <option value="salary">💰 Salary</option>
            <option value="Changed password">🔑 Password</option>
            <option value="system user">👤 System User</option>
            <option value="department">🏢 Department</option>
          </select>
          <select class="flt" id="ausr" onchange="rAudit()">
            <option value="">All Users</option>
          </select>
          <select class="flt" id="adr" onchange="rAudit()">
          ...
          <div class="sp"></div>
          <span id="audit-count" style="font-size:12px;color:var(--txm);align-self:center"></span>
          <button class="btn bh sm" onclick="expAuditCSV()">📤 Export CSV</button>
        </div>
        <div class="tw"><table><thead><tr>
          <th>Timestamp</th><th>User</th><th>Action</th><th>Target</th><th>Detail</th>
      ...
      <section class="pg" id="pg-sysusers">
        ...
        <div class="tw"><table><thead><tr><th>Username</th><th>Full Name</th><th>Role</th><th>Department Scope</th><th>Employee ID</th><th>Created</th><th></th></tr></thead><tbody id="sutb"></tbody></table></div>
      </section>
...
<div class="mo2" id="m-su"><div class="mdl">
  ...
  <div class="fg">
    ...
    <div class="fgr"><label class="flb">Department <span style="color:var(--txm);font-size:10px">(scopes visibility for Manager/Staff)</span></label>
      <select class="fsel" id="su-dept">
...
const RL={ceo:6,dg:5,admin:4,hr:3,manager:2,staff:1};
const RLAB={ceo:'CEO',dg:'Director General',admin:'Admin',hr:'HR Manager',manager:'Manager',staff:'Staff'};
const RICO={ceo:'👑',dg:'🎖️',admin:'🛡️',hr:'👔',manager:'📋',staff:'🧑‍💼'};
const RCOL={ceo:'#f5c518',dg:'#c084fc',admin:'#e05252',hr:'#4f8ef7',manager:'#e09a30',staff:'#3dba7a'};
...
function rPay(){
  ...
  const fd=document.getElementById('pfd')?.value||'';
  const fc=document.getElementById('pfc')?.value||'';
  const fs=document.getElementById('pfs')?.value||'';
  const lvl=RL[S.user.role]||1;
  if(lvl<3){document.getElementById('prtb').innerHTML='<tr class="er"><td colspan="9">Access restricted to HR and Admin.</td></tr>';return;}

  // populate dept filter
  const pfdSel=document.getElementById('pfd');
  if(pfdSel){
    const cur=pfdSel.value;
    pfdSel.innerHTML='<option value="">All Departments</option>'+
      Object.keys(S.departments).map(d=>`<option value="${esc(d)}" ${d===cur?'selected':''}>${esc(d)}</option>`).join('');
    if(cur)pfdSel.value=cur;
  }

  const allEmps=Object.entries(S.employees);
  const filtered=allEmps.filter(([id,e])=>{
    if(fd&&e.department!==fd)return false;
    if(fc&&e.salary_currency!==fc)return false;
    if(fs&&e.status!==fs)return false;
    if(q&&![id,e.full_name,e.department,e.occupation,e.role].some(x=>(x||'').toLowerCase().includes(q)))return false;
    return true;
  });

  // ── Stats
  let totUSD=0,withSal=0,highSal=0,lowSal=Infinity;
  const byCurr={};
  Object.values(S.employees).forEach(e=>{
    if(!e.salary)return;
    const n=Number(e.salary);
    if(e.salary_currency==='USD'){totUSD+=n;withSal++;if(n>highSal)highSal=n;if(n<lowSal)lowSal=n;}
    byCurr[e.salary_currency]=(byCurr[e.salary_currency]||0)+1;
  });
  const avgUSD=withSal?Math.round(totUSD/withSal):0;
  if(lowSal===Infinity)lowSal=0;
  document.getElementById('prstats').innerHTML=[
    {label:'Total Employees',val:Object.keys(S.employees).length,cls:''},
    {label:'With Salary Set',val:withSal,cls:'gn'},
    {label:'Total USD Payroll',val:'$'+totUSD.toLocaleString(),cls:'bl'},
    {label:'Avg USD Salary',val:'$'+avgUSD.toLocaleString(),cls:''},
    {label:'Highest USD',val:highSal?'$'+highSal.toLocaleString():'—',cls:'gn'},
    {label:'Lowest USD',val:lowSal?'$'+lowSal.toLocaleString():'—',cls:'rd'},
  ].map(s=>`<div class="sc ${s.cls}"><div class="sv" style="font-size:${typeof s.val==='string'&&s.val.length>8?'22px':'34px'}">${s.val}</div><div class="slb">${s.label}</div></div>`).join('');
  ...
  if(Object.keys(deptPay).length){
    deptBreakEl.innerHTML=`<div class="card" style="margin-bottom:0"><div class="ct">USD Payroll by Department</div><div style="display:flex;gap:14px;flex-wrap:wrap">${
      Object.entries(deptPay).sort(([,a],[,b])=>b-a).map(([d,v])=>{
        ...
        const pct=totUSD?Math.round(v/totUSD*100):0;
        return `<div style="flex:1;min-width:140px">
          <div style="font-size:11px;color:var(--txm);margin-bottom:3px">${esc(def.icon||'🏢')}&nbsp;${esc(d)}</div>
          <div style="font-family:var(--bb);font-size:18px;color:var(--gold)">$${v.toLocaleString()}</div>
          <div style="height:5px;background:var(--bdr);border-radius:3px;margin-top:4px">
            <div style="height:5px;background:${esc(def.color||'#c9a84c')};border-radius:3px;width:${pct}%;transition:width .4s"></div>
          </div>
          <div style="font-size:10px;color:var(--txm);margin-top:3px">${pct}% · $${v.toLocaleString()}</div>
        </div>`;
      }).join('')
    }</div></div>`;
  ...
  const salBands={'<20k':0,'20–50k':0,'50–100k':0,'100–200k':0,'200k+':0};
  Object.values(S.employees).forEach(e=>{
    if(!e.salary||e.salary_currency!=='USD')return;
    const n=Number(e.salary);
    if(n<20000)salBands['<20k']++;
    else if(n<50000)salBands['20–50k']++;
    else if(n<100000)salBands['50–100k']++;
    else if(n<200000)salBands['100–200k']++;
    else salBands['200k+']++;
  });
  const chartEl=document.getElementById('pay-chart');
  if(chartEl){
    if(S.charts['pay']){S.charts['pay'].destroy();delete S.charts['pay'];}
    S.charts['pay']=new Chart(chartEl,{
      type:'bar',
      data:{labels:Object.keys(salBands),datasets:[{data:Object.values(salBands),
        backgroundColor:['#4f8ef7','#3dba7a','#c9a84c','#e09a30','#e05252'],
        borderWidth:0,borderRadius:6}]},
      options:{responsive:true,plugins:{legend:{display:false},
        tooltip:{callbacks:{label:ctx=>`${ctx.raw} employee${ctx.raw!==1?'s':''}`}}},
        scales:{y:{grid:{color:'rgba(255,255,255,0.04)'},ticks:{color:'#5a6080',stepSize:1}},
                x:{grid:{display:false},ticks:{color:'#5a6080'}}}}
    });
  }

  // ── Count label
  const countEl=document.getElementById('pay-count');
  if(countEl)countEl.textContent=`${filtered.length} of ${allEmps.length} employees`;

  // ── Table
  const tb=document.getElementById('prtb');
  if(!filtered.length){tb.innerHTML='<tr class="er"><td colspan="9">No employees match the filters.</td></tr>';return;}
  tb.innerHTML=filtered.map(([id,e])=>{
    const sc=stCls(e.status);
    const hasSal=!!e.salary;
    return `<tr class="${S.selPay===id?'sel':''}" onclick="selPayRow('${esc(id)}',this)">
      <td style="font-family:var(--mo);font-size:11px">${esc(id)}</td>
      <td><strong>${esc(e.full_name||'')}</strong></td>
      <td>${esc(e.department||'—')}</td>
      <td style="color:var(--txm);font-size:12px">${esc(e.occupation||'—')}</td>
      <td><span class="bdg bgray" style="font-size:10px">${esc(e.role||'—')}</span></td>
      <td class="saldsp" style="font-size:14px;${hasSal?'':'color:var(--txm)'}">
        ${hasSal?`<strong>${esc(e.salary_currency)}</strong> ${Number(e.salary).toLocaleString()}`:'—'}
      </td>
      <td style="font-size:11px;color:var(--txm)">${esc(e.salary_currency||'—')}</td>
      <td style="font-size:11px">${esc((ET[e.employment_type]||e.employment_type||'—'))}</td>
      <td><span class="bdg ${sc}" style="font-size:10px">${esc(e.status||'—')}</span></td>
    </tr>`;
  }).join('');
}
...
function rAudit(){
  ...
  const ausr=document.getElementById('ausr')?.value||'';
  const adr=document.getElementById('adr')?.value||'';

  // populate user filter
  const usrSel=document.getElementById('ausr');
  if(usrSel){
    const cur=usrSel.value;
    const users=[...new Set(S.auditLog.map(a=>a.user).filter(Boolean))].sort();
    usrSel.innerHTML='<option value="">All Users</option>'+users.map(u=>`<option value="${esc(u)}" ${u===cur?'selected':''}>${esc(u)}</option>`).join('');
    if(cur)usrSel.value=cur;
  }

  // date range filter
  const now=new Date();
  let cutoff=null;
  if(adr==='today'){cutoff=new Date(now.getFullYear(),now.getMonth(),now.getDate());}
  else if(adr==='week'){cutoff=new Date(now-7*864e5);}
  else if(adr==='month'){cutoff=new Date(now.getFullYear(),now.getMonth(),1);}

  const rows=S.auditLog.filter(a=>{
    if(af&&!a.action.toLowerCase().includes(af.toLowerCase()))return false;
    if(ausr&&a.user!==ausr)return false;
    if(cutoff&&new Date(a.ts)<cutoff)return false;
    if(q&&![a.user,a.action,a.target,a.detail,a.ts].some(x=>(x||'').toLowerCase().includes(q)))return false;
    return true;
  });

  // ── Stats bar
  const byAction={};
  S.auditLog.slice(0,500).forEach(a=>{
    const key=a.action.split(' ')[0];
    byAction[key]=(byAction[key]||0)+1;
  });
  const topActions=Object.entries(byAction).sort(([,a],[,b])=>b-a).slice(0,5);
  document.getElementById('auditstats').innerHTML=[
    {label:'Total Events',val:S.auditLog.length,cls:''},
    {label:'Showing',val:rows.length,cls:'gn'},
    {label:'Unique Users',val:new Set(S.auditLog.map(a=>a.user)).size,cls:'bl'},
    {label:'Today',val:S.auditLog.filter(a=>a.ts&&a.ts.startsWith(now.toISOString().slice(0,10))).length,cls:'wn'},
  ].map(s=>`<div class="sc ${s.cls}"><div class="sv">${s.val}</div><div class="slb">${s.label}</div></div>`).join('');

  // ── Count
  const cntEl=document.getElementById('audit-count');
  if(cntEl)cntEl.textContent=`${rows.length} event${rows.length!==1?'s':''}`;

  const ACTION_ICO={
    'Login':'🔐','Added':'➕','Updated':'✏️','Deleted':'🗑',
    'Promotion':'🏆','Leave':'📅','Imported':'📥','Changed':'🔑',
    'Saved':'💾','Updated salary':'💰','Exported':'📤'
  };
  const ACTION_CLS={
    'Login':'bblue','Added employee':'bgreen','Deleted employee':'bred',
    'Updated employee':'bwarn','Imported employees':'bpurple',
    'Promotion approved':'bgreen','Promotion denied':'bred',
    'Leave approved':'bgreen','Leave denied':'bred',
    'Changed password':'bgray','Updated salary':'bgold',
    'Saved department':'bblue','Deleted department':'bred',
    'Saved system user':'bblue','Deleted system user':'bred',
  };

  const tb=document.getElementById('atb');
  if(!rows.length){tb.innerHTML='<tr class="er"><td colspan="5">No log entries match the filters.</td></tr>';return;}
  tb.innerHTML=rows.map(a=>{
    const aCls=ACTION_CLS[a.action]||'bgray';
    const ico=ACTION_ICO[a.action]||ACTION_ICO[a.action?.split(' ')[0]]||'📋';
    const userCol=RCOL[S.sysUsers?.[a.user]?.role]||'#c9a84c';
    return `<tr>
      <td style="font-family:var(--mo);font-size:11px;color:var(--txm);white-space:nowrap">${esc(a.ts)}</td>
      <td><span class="bdg" style="background:${esc(userCol)}22;color:${esc(userCol)};border:1px solid ${esc(userCol)}33;font-size:11px">${esc(a.user||'—')}</span></td>
      <td><span class="bdg ${aCls}" style="font-size:10px">${ico} ${esc(a.action)}</span></td>
      <td style="font-family:var(--mo);font-size:11px;color:var(--txm)">${esc(a.target)}</td>
      <td style="font-size:12px;color:var(--txm);max-width:260px;overflow:hidden;text-overflow:ellipsis;white-space:nowrap" title="${esc(a.detail||'')}">${esc(a.detail||'—')}</td>
    </tr>`;
  }).join('');
}
...
function rRoles(){
  ...
  document.getElementById('role-cards-container').innerHTML=`
    <div class="role-grid">
      ${['ceo','dg','admin','hr','manager','staff'].map(k=>{
        const def=defs[k]||{};
    ...
    <div class="card">
      ...
      <table>
        <thead><tr><th>Feature</th><th>Staff</th><th>Manager</th><th>HR</th><th>Admin</th><th>DG</th><th>CEO</th></tr></thead>
        <tbody>
          ${[
            ['View own record',         '✅','✅','✅','✅','✅','✅'],
            ['View dept employees',      '','✅','✅','✅','✅','✅'],
            ...
            ['Manage system users',      '','','','✅','',''],
            ['Change own password',      '✅','✅','✅','✅','✅','✅'],
          ].map(([feat,...perms])=>`<tr><td>${feat}</td>${perms.map(p=>`<td style="text-align:center">${p||'<span style=\"color:var(--bdr)\">—</span>'}</td>`).join('')}</tr>`).join('')}
        </tbody>
      </table>
      </div>
    </div>`;
...
function rSU(){
  ...
  if(!Object.keys(S.sysUsers).length){tb.innerHTML='<tr class="er"><td colspan="7">No system users found.</td></tr>';return;}
  tb.innerHTML=Object.entries(S.sysUsers).map(([u,d])=>{
    const def=S.roleDefs[d.role]||{};
    const col=def.color||RCOL[d.role]||'#c9a84c';
    const roleBadge=`<span class="bdg" style="background:${esc(col)}22;color:${esc(col)};border:1px solid ${esc(col)}44">${esc(def.icon||RICO[d.role]||'')} ${esc(RLAB[d.role]||d.role)}</span>`;
    const deptBadge=d.department?`<span class="bdg bgray" style="font-size:10px">${esc(d.department)}</span>`:'<span style="color:var(--txm);font-size:11px">All depts</span>';
    return `<tr>
...
function populateSUDeptDropdown(selectedDept){
  const sel=document.getElementById('su-dept');
  sel.innerHTML='<option value="">— No restriction (all departments) —</option>'+
    Object.keys(S.departments).map(d=>`<option value="${esc(d)}" ${d===selectedDept?'selected':''}>${esc(d)}</option>`).join('');
}
function oAddSU(){
...
function oEditSU(u){
  ...
  populateSUDeptDropdown(d.department||'');
  updateRolePreview();
  openModal('m-su');
}
...
async function saveSU(){
  const orig=v('su-ou'),uname=v('su-un'),fn=v('su-fn');
  const role=document.getElementById('su-rl').value;
  const dept=document.getElementById('su-dept').value||null;
  const eid=v('su-eid'),pw=v('su-pw');
  if(!uname){toast('Username required.','error');return;}
  const body={username:uname,full_name:fn,role,emp_id:eid||null,department:dept};
...
def get_data():
    ...
    # ── Salary visibility: only HR+ can see salary fields
    if lvl < 3:
        emps = {k: {f: v for f, v in e.items() if f not in ("salary","salary_currency")}
                for k, e in emps.items()}

    # ── Department scope: CEO/DG/Admin/HR see everyone.
    #    Manager/Staff are scoped to their department (derived from linked employee OR
    #    explicit department set on the system user record).
    if lvl < 3:
        # Determine the user's department
        user_dept = u.get("department") or None
        if not user_dept and u.get("emp_id"):
            linked = data["employees"].get(u["emp_id"], {})
            user_dept = linked.get("department")

        if lvl == 2:
            # Manager: sees own department; if no dept set, sees only own record
            if user_dept:
                emps = {k: v for k, v in emps.items() if v.get("department") == user_dept}
            else:
                emp_id = u.get("emp_id")
                emps = {k: v for k, v in emps.items() if k == emp_id}
        else:
            # Staff: always only own record
            emp_id = u.get("emp_id")
            emps = {k: v for k, v in emps.items() if k == emp_id}
    ...
    result = {
        ...
        "audit_log": data.get("audit_log", [])[:500] if lvl >= 3 else [],
    ...
    if lvl >= 4:
        result["system_users"] = {
            u2: {k: v2 for k, v2 in d2.items() if k != "password_hash"}
            for u2, d2 in data["system_users"].items()
        }
...
def update_sysuser(uname):
    ...
    u["department"] = body.get("department") if "department" in body else u.get("department")
